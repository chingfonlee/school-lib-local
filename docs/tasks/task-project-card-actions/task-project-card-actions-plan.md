...
只加兩行到現有 `.project-card` 規則，並新增 hover rule：

```css
/* 修改既有 .project-card：加上 cursor + transition */
.project-card {
  /* ...現有屬性不變... */
  cursor: pointer;               /* ← 新增 */
  transition: box-shadow 0.15s;  /* ← 新增 */
}
...
- 不加 border（避免 layout shift）
- 不加 background 顏色變化（延續白卡片風格）
- 不加 transform/scale（不過度花俏）
...
### Step 1：CSS — `.project-card` 加 hover 提示
...
### Step 2：HTML — delete modal 新增警告 div
...
### Step 3：HTML — 卡片加 onclick，按鈕傳 event
...
### Step 4：JS — 新增 `selectCard`，更新三個函式
...
| 驗證項目 | 步驟 | 預期結果 |
|---------|------|---------|
| 卡片 hover 提示 | 滑鼠移至卡片文字區 | cursor 變 pointer，box-shadow 加深 |
