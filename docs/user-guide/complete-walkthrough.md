...
**適用對象：** 國小圖書館管理員、行政人員或負責採購業務的教師。本文件假設您已完成系統安裝與設定。若尚未安裝，請先參閱 [Windows 安裝指南](install-windows.md)。
...
> **注意：**
> - 匯出範本需在 `config.yaml` 中正確設定，且 `00_source/` 資料夾內需有對應的空白 Excel 範本檔案。詳情請見 [Windows 安裝指南](install-windows.md)。
...
- 確認 `config.yaml` 中 `export_templates` 設定的 `template_file_path` 路徑正確
- 確認 `00_source/` 資料夾內有對應的空白 Excel 範本檔案
- 範本路徑為本機相對路徑，更換電腦時需重新設定
...
目前系統預設搭配特定縣市的採購書單範本（Excel 格式）。若需切換為其他範本，請修改 `config.yaml` 中的 `export_templates` 設定，並將對應的空白範本 Excel 放置於 `00_source/` 資料夾。詳細設定說明請參閱 [Windows 安裝指南](install-windows.md)。
